            ...
            try:
                ...
                for tk_item in [x for x in tk_details["price_list"] if x["close"] != 0]:
                    if time_cmp(str(date), tk_item["date"]):
                        price_list.append(tk_item["close"])
                        tur_list.append(tk_item["volume"])
                        highest_list.append(tk_item["high"])
                        open_list.append(tk_item["open"])
                print("price_list", code, price_list)
                wmacd_list, diff_list, dea_list = self.get_w_macd(price_list[:])
                print("wmacd_list", wmacd_list)
                print("diff_list", diff_list)
                print("dea_list", dea_list)
                # 创建wmacd实体
